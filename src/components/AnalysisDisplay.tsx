  ...
  const processedLines = result.split("\n").map((line, index) => {
    ...
    if (line.startsWith("### ")) {
      return (
        <h3
          key={index}
          className="text-lg font-semibold text-neutral-700 mt-3 mb-1"
        >
          {line.substring(4)}
        </h3>
      );
    ...
    if (line.startsWith("# ")) {
      return (
        <h1
          key={index}
          className="text-2xl font-bold text-neutral-800 mt-5 mb-3"
        >
          {line.substring(2)}
        </h1>
      );
    ...
    return (
      <p key={index} className="text-neutral-700 leading-relaxed my-1">
        {line}
      </p>
    );
  });

  const formattedResult = processedLines.reduce<React.ReactNode[]>(
    (acc, currNode, idx) => {
      ...
      if (React.isValidElement(currNode) && currNode.type === "li") {
        ...
        if (
          lastElementInAcc &&
          React.isValidElement(lastElementInAcc) &&
          lastElementInAcc.type === "ul"
        ) {
          // It is, so append the current LI to this UL's children
          const ulElement = lastElementInAcc as React.ReactElement<
          ...
          acc[acc.length - 1] = React.cloneElement(ulElement, {
            ...ulElement.props,
          ...
          acc.push(
            <ul key={`ul-${idx}`} className="list-disc pl-5 my-2 ml-5">
              {[currNode]}
            </ul>
          ); // Added ml-5 to ul for consistent indentation with li
        }
      } else {
